...
- **做 A 股短中线研究的交易者**：希望把市场观察、个股分析、计划、执行、复盘放到一套系统里
- **关注交易纪律而不是只看买卖点的人**：希望研究结论能落到交易计划、日志和复盘
- **想把本地模型真正用进业务流程的开发者**：不满足于“聊天框 + Prompt”，而是希望做成可追踪的工作流
- **对多 Agent / 多 MCP 编排感兴趣的工程师**：想看一个垂直领域里，Agent、工具、状态机和桌面应用如何结合

## 为什么现在值得看

`StockCopilot` 的价值不只是功能堆得多，而是它已经把几件难同时做好的事情做到了同一条链里：
