...
public static class LocalFactSchemaInitializer
{
    public static async Task EnsureAsync(AppDbContext dbContext, CancellationToken cancellationToken = default)
    {
        ...
        await dbContext.Database.ExecuteSqlRawAsync("IF COL_LENGTH('dbo.LocalStockNews','ArticleExcerpt') IS NULL ALTER TABLE dbo.LocalStockNews ADD ArticleExcerpt NVARCHAR(MAX) NULL;", cancellationToken);
        await dbContext.Database.ExecuteSqlRawAsync("IF COL_LENGTH('dbo.LocalStockNews','ArticleSummary') IS NULL ALTER TABLE dbo.LocalStockNews ADD ArticleSummary NVARCHAR(MAX) NULL;", cancellationToken);
        await dbContext.Database.ExecuteSqlRawAsync("IF COL_LENGTH('dbo.LocalStockNews','ReadMode') IS NULL ALTER TABLE dbo.LocalStockNews ADD ReadMode NVARCHAR(32) NOT NULL CONSTRAINT DF_LocalStockNews_ReadMode DEFAULT N'url_unavailable';", cancellationToken);
        await dbContext.Database.ExecuteSqlRawAsync("IF COL_LENGTH('dbo.LocalStockNews','ReadStatus') IS NULL ALTER TABLE dbo.LocalStockNews ADD ReadStatus NVARCHAR(32) NOT NULL CONSTRAINT DF_LocalStockNews_ReadStatus DEFAULT N'unverified';", cancellationToken);
        await dbContext.Database.ExecuteSqlRawAsync("IF COL_LENGTH('dbo.LocalStockNews','IngestedAt') IS NULL ALTER TABLE dbo.LocalStockNews ADD IngestedAt DATETIME2 NULL;", cancellationToken);
        ...
        await dbContext.Database.ExecuteSqlRawAsync("IF COL_LENGTH('dbo.LocalSectorReports','ArticleExcerpt') IS NULL ALTER TABLE dbo.LocalSectorReports ADD ArticleExcerpt NVARCHAR(MAX) NULL;", cancellationToken);
        await dbContext.Database.ExecuteSqlRawAsync("IF COL_LENGTH('dbo.LocalSectorReports','ArticleSummary') IS NULL ALTER TABLE dbo.LocalSectorReports ADD ArticleSummary NVARCHAR(MAX) NULL;", cancellationToken);
        await dbContext.Database.ExecuteSqlRawAsync("IF COL_LENGTH('dbo.LocalSectorReports','ReadMode') IS NULL ALTER TABLE dbo.LocalSectorReports ADD ReadMode NVARCHAR(32) NOT NULL CONSTRAINT DF_LocalSectorReports_ReadMode DEFAULT N'url_unavailable';", cancellationToken);
        await dbContext.Database.ExecuteSqlRawAsync("IF COL_LENGTH('dbo.LocalSectorReports','ReadStatus') IS NULL ALTER TABLE dbo.LocalSectorReports ADD ReadStatus NVARCHAR(32) NOT NULL CONSTRAINT DF_LocalSectorReports_ReadStatus DEFAULT N'unverified';", cancellationToken);
        await dbContext.Database.ExecuteSqlRawAsync("IF COL_LENGTH('dbo.LocalSectorReports','IngestedAt') IS NULL ALTER TABLE dbo.LocalSectorReports ADD IngestedAt DATETIME2 NULL;", cancellationToken);
