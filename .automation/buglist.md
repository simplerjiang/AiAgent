...
| **#114** | `fundamental-snapshot` 端点在无缓存数据时返回 204 No Content 而非 404，消除控制台红色日志 | ✓ FIXED |
| **#115** | `StockMarketDataSchemaInitializer` 添加 `CREATE TABLE IF NOT EXISTS StockQueryHistories` | ✓ FIXED |
| **#116** | `/api/news` handler 分离 EnsureFresh 超时 token 与 Query token，fallback 到缓存数据 | ✓ FIXED |
| **#117** | `IntradayMessagesEndpointTimeout` 从 3s 提升到 8s | ✓ FIXED |
| **#119** | 回测仪表板添加小样本警告 | ✓ FIXED |
| **#121** | 回测记分卡添加准确率计算规则说明 | ✓ FIXED |
| **#123** | 批量回测"处理 0 条"添加引导文案 | ✓ FIXED |
